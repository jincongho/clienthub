...
{option:!tooltip}<h2>Create a new Profile</h2>{/option:!tooltip}
{form:newProfile}
	<p> 
...
$().ready(function(){
	$("#header ul li:contains('Home')").addClass('selected');
	...
	$("input[id=name]").keyup(function(){
		...
		request = $.ajax({
			  ...
			  success: function(json){
				if(json.count > 1){
					$("#userexist").html(json.count + " users in db have same name. Search Them <a href='{$base_url}/search.php' target='_blank'>Here</a>!");
					$("#userexist").fadeIn();
				}else if(json.count == 1){
					$("#userexist").html(json.count + " user in db have same name. Check it out <a href='{$base_url}/edit.php?id=" + json.profiles + "' target='_blank'>here</a>!");
					$("#userexist").fadeIn();
				}else{
